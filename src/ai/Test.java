	...
	public static MultiLayerPerceptron learn(int size, HashMap<Integer, Coup> mapTrain, int h, double lr, int l, boolean verbose, double epochs){
		try {
			...
			for(int i = 0; i < epochs; i++){
				...
				if ( i % 10000 == 0 && verbose) System.out.println("Error at step "+i+" is "+ (error/i));
			}
			if ( verbose )
	...
	public static void test(int size, int h, double lr, int l){
		try {
			...
			for(int i = 0; i < epochs; i++){
				...
				if ( i % 10000 == 0 ) System.out.println("Error at step "+i+" is "+ (error/i));
			}
	...
	public static void test(){
		try {
			...
			for(int i = 0; i < samples; i++){
				...
				if ( i % 100000 == 0 ) System.out.println("Error at step "+i+" is "+ (error/i));
			}
			error /= samples ;
